...
const Loading = ({ setLoading }) => {
  const [state, setState] = useState({ pathIndex: 0 });

  const gotoNext = () => {
    const { pathIndex } = state;
  ...
  const autoNext = () => {
    let nextIndex = 0;
    const maxSvgs = pathIds.length - 1;
    const timer = setInterval(() => {
      nextIndex++;
      setState({
        pathIndex: nextIndex > maxSvgs ? 0 : nextIndex
      });
      if (nextIndex > maxSvgs) {
        clearTimeout(timer);
        setLoading(true);
      }
    }, 500);
  ...
  return (
    <div>
      <svg width="400" height="400" viewBox="0 0 520 500">
        <Icon pose={pathIds[state.pathIndex]} />
      </svg>
